...
function stageNote() {
  ...
  try {
    var length = Module._fm_stage_len ? Module._fm_stage_len() : 0;
    if (!length) return '';
    var pointer = Module._fm_stage_ptr();
...
function runtimeError() {
  var length = Module._fm_error_len();
  if (!length) {
    var stage = stageNote();
    return stage
  ...
  var pointer = Module._fm_error_ptr();
  return new TextDecoder().decode(
    Module.HEAPU8.slice(pointer, pointer + length));
}
...
function ingestPacket() {
  var length = Module._fm_packet_len();
  if (!length) throw new Error('Replay runtime produced an empty frame');
  var pointer = Module._fm_packet_ptr();
...
function sendRuntimeInput(bytes) {
  ...
  copyIntoRuntime(bytes, function (pointer, length) {
    Module._fm_input(pointer, length);
  });
}
...
async function start() {
  ...
  try {
    ...
    var loaded = copyIntoRuntime(bytes, function (pointer, length) {
      return Module._fm_load_replay(pointer, length);
    });
    if (!loaded) throw new Error(runtimeError());
    runtimeLoaded = true;
    ingestPacket();
    // No mismatch tick: Fruit Market records STATE, not inputs, so playback
    // never re-simulates and there is nothing to mismatch.
    postMessage({ type: 'loaded', mismatchTick: -1 });
  } catch (error) {
    reportFailure(error);
  }
}
...
function advance(frames) {
  ...
  try {
    ...
    for (var i = 0; i < count; i++) {
      if (Module._fm_frame() < 0) throw new Error(runtimeError());
      ingestPacket();
    }
    postMessage({
      type: 'advanced',
...
importScripts('./wire_constants.js', './broadcast_core.js', './fruit_market_replay.js');
