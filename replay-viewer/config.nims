...
# use-after-free fixed in fruit_market_replay.nim (emscripten_exit_with_live_runtime)
...
# fm_error_len(). Aborting keeps linear memory intact, and the page reads
# fm_stage_ptr/len afterwards to report what the runtime was doing.
switch(
  "passL",
  (&"""
  -o {distDir / "fruit_market_replay.js"}
  ...
  -s EXPORTED_FUNCTIONS=_main,_malloc,_free,_fm_load_replay,_fm_frame,_fm_input,_fm_packet_ptr,_fm_packet_len,_fm_error_ptr,_fm_error_len,_fm_stage_ptr,_fm_stage_len
  """).replace("\n", " ")
)
