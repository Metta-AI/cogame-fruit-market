...
# The hook resolves its output path by cd-ing into the parent, which
# `coworld build` pre-creates but CI does not (ecos, 2026-08-23). Create it
# before the containment check rather than exiting 1 on a fresh checkout.
...
image_tag="cogame-fruit-market-replay-viewer-build:$$"
container_id=""
cleanup() {
...
docker cp "${container_id}:/workspace/fruit-market/replay-viewer/dist/." "${output_dir}"
